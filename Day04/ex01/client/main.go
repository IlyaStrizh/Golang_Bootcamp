// curl -s --key ../minica/client/key.pem --cert ../minica/client/cert.pem --cacert ../minica/minica.pem -XPOST -H "Content-Type: application/json" -d '{"candyType": "NT", "candyCount": 1, "money": 34}' "https://localhost:3333/buy_candy"
...
func main() {
	...
	cert, err := tls.LoadX509KeyPair("ex01/minica/client/cert.pem", "ex01/minica/client/key.pem")
	if err != nil {
		log.Fatal("Failed to load client certificate and key:", err)
	}

	// Загрузка CA certificate
	caCert, err := os.ReadFile("ex01/minica/minica.pem")
	if err != nil {
